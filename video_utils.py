...
def frames_to_ascii(frames_path, new_frames_save_path, kwargs, type='mono', max_workers=20):
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as pool:
        ...
        for frame in frames:
            ...
            if type == 'mono':
                all_task.append(pool.submit(mono_ret_image, frame, output_image, **kwargs))
            elif type == 'color':
                all_task.append(pool.submit(color, frame, output_image, **kwargs))
            else:
                print('请指定正确的转换类型')

        with tqdm(desc=f'Image2ascii: ', ncols=80, total=len(frames)) as pbar_b:
