...
if __name__ == '__main__':
    ...
    # test for HD images
    input_image = 'test_imgs/kuaishou/standard_images/p12_c.jpg'
    output_image = f"{input_image.split('.')[0]}_color_output.{input_image.split('.')[-1]}"
    
    # 英文字符通用参数参考
    # kwargs = {
    #     'rows': 100,
    #     'alphabet': 'uppercase',          # 字符
    ...
    kwargs = {
        ...
        'background': 'origin6',            # 背景色，数字表示不透明度，可以用来控制图片亮度
        'out_height': None,
        'fontsize': 17,                     # 中文自定义需要手动调整字体大小已获得一个比较好的效果
        'hw_ratio': 1.25,
        'char_width': 8.8,
        ...
        'char_width_gap_ratio': 1.8,        # 中文字符间隔需要手动调整，防止拥挤
        'char_height_gap_ratio': 1.8,
    }
